...
- [x] Fase 3 — Timeline Pelaksanaan (kode selesai termasuk revisi: flag status milestone, tampilan mingguan, edit-lock, dan bidang untuk leader_bidang; `next build`/`next lint`/`tsc --noEmit` sukses di sandbox. **Ada 1 migration SQL baru** di revisi fase ini (`20260809000004_fase3_committee_bucket_assignment.sql`) — belum dijalankan/diverifikasi live oleh Yolanda, lihat catatan di bawah)
- [ ] Fase 4 — Bucket Tugas & Breakdown
- [ ] Fase 5 — Download/Submit Template Budgeting
- [ ] Fase 6 — Dashboard Kepanitiaan (Leader)
- [ ] Fase 7 — Notifikasi
...
4. **Leader Bidang bisa pilih bidang** — field baru `committee_members.bucket_id` (nullable, FK ke `buckets.id`, `on delete set null`), ditambah lewat migration baru `20260809000004_fase3_committee_bucket_assignment.sql` (bukan bagian dari migration Fase 1, karena kolom ini memang belum ada saat itu). Selain kolom, migration ini juga nambah trigger `trg_check_committee_member_bucket_instance` yang memvalidasi `bucket_id` yang dipilih harus milik `kepanitiaan_site_id` yang sama dengan baris `committee_members`-nya — defense-in-depth di level DB, karena UI cuma menawarkan pilihan bucket dari instance yang benar tapi tidak ada yang mencegah request manual langsung ke Supabase REST API mengisi `bucket_id` dari instance lain.
   - **Dropdown "Bidang" selalu ditampilkan di form**, terlepas dari `role` yang dipilih (bukan disembunyikan/dimunculkan pakai JS tergantung pilihan role) — mengikuti prinsip section susunan panitia yang sengaja tidak butuh client-side JS. Validasinya dipindah ke server: `lib/committee/actions.ts` punya helper baru `parseBucketId(role, value)` yang **selalu me-null-kan** `bucket_id` kalau `role !== "leader_bidang"`, jadi tidak ada `bucket_id` "nyangkut" kalau seseorang di-downgrade dari leader_bidang ke anggota.
...
7. Ubah role anggota yang sudah punya bidang balik ke "Anggota" — simpan — pastikan `bucket_id`-nya ikut hilang (buka lagi form-nya, dropdown Bidang harus balik ke "- (bukan leader bidang)").
8. Di card Timeline, coba klik tab "Mingguan" dan "Vertikal" bolak-balik, pastikan pengelompokan minggunya masuk akal. Coba klik "Edit" pada satu milestone, ubah tanggal, "Simpan" — pastikan baris otomatis balik ke mode terkunci (bukan tetap dalam mode form). Cek juga badge status berubah otomatis sesuai tanggal hari ini (misal buat milestone dengan `tanggal_mulai` besok vs hari ini vs kemarin, badgenya harus beda: "Akan Datang"/"Berlangsung"/"Sudah Lewat").
