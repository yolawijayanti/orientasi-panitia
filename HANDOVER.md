...
Yang **masih belum bisa dikonfirmasi** (bukan karena bug, tapi karena akun test-nya sendiri bermasalah): isolasi RLS `timeline_milestones_scoped` dari sisi akun **panitia** (login sebagai panitia instance A, coba akses `/panitia/timeline`, pastikan tidak lihat data instance B). Risikonya dinilai rendah karena mengikuti pola RLS `is_leader() OR kepanitiaan_site_id = current_user_kepanitiaan_site_id()` yang sama seperti `buckets`/`committee_members` yang sudah teruji isolasinya sejak Fase 1, tapi belum benar-benar dicoba ulang di Fase 3.

**Kendala saat mencoba: login `panitia-a@test.local`/`panitia-b@test.local` ditolak.** Akun-akun ini awalnya dibuat khusus untuk verifikasi RLS Fase 1 (lihat `verify_rls_fase1.sql`), dan HANDOVER Fase 1 sudah mencatat data test `*@test.local` dibersihkan Yolanda setelah verifikasi Fase 1 selesai — kemungkinan besar baris `public.users` untuk kedua akun ini (dan instance test lama yang jadi rujukan `kepanitiaan_site_id`-nya) sudah ikut terhapus, sementara akun login-nya sendiri di Supabase Auth (`auth.users`) mungkin masih ada atau mungkin juga sudah tidak ada — belum dipastikan yang mana. Kalau baris `auth.users` masih ada tapi `public.users` hilang, gejalanya persis "ditolak" tanpa error jelas: Supabase Auth berhasil, tapi `lib/auth/actions.ts` `login()` menolak dengan pesan "Akun ini belum terdaftar di sistem orientasi panitia" karena tidak nemu baris `public.users`-nya. **Belum ada perbaikan kode untuk ini** karena memang bukan bug kode — solusinya menyiapkan ulang kedua akun test itu (Auth + baris `public.users`, dihubungkan ke 2 instance ASLI yang sekarang ada, bukan instance test lama yang sudah hilang). Langkah lengkapnya sudah ditulis di `supabase/verify_fase3.sql` (dibuat di revisi ini, pola sama seperti `verify_rls_fase1.sql`/`verify_fase2.sql`). Kalau tidak sempat menutup celah ini, boleh lanjut ke Fase 4 dulu dan cek ini belakangan — risikonya kecil karena pola RLS-nya identik dengan yang sudah terbukti aman di tabel lain.
