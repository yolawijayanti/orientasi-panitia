...
- [ ] Fase 4 — Bucket Tugas & Breakdown (kode & struktur selesai di sandbox — `next build`/`next lint`/`tsc --noEmit` sukses, semua route baru terdaftar — tapi **belum diverifikasi live oleh Yolanda**, jadi belum dicentang. Lihat daftar langkah verifikasi live di Catatan Teknis Fase 4 di bawah sebelum lanjut ke Fase 5)
- [ ] Fase 5 — Download/Submit Template Budgeting
- [ ] Fase 6 — Dashboard Kepanitiaan (Leader)
- [ ] Fase 7 — Notifikasi
- [ ] Fase 8 — Polish Desain

**Fase berikutnya yang harus dikerjakan: selesaikan verifikasi live Fase 4, baru lanjut ke Fase 5.** Kode Fase 4 (Bucket Tugas & Breakdown) sudah dibangun di branch ini — tidak ada migration SQL baru yang perlu dijalankan (tabel `tasks`/`subtasks` dan RLS-nya sudah ada dari Fase 1), jadi begitu PR ini di-merge, langkah yang perlu Yolanda lakukan cuma klik-klik manual sesuai daftar di Catatan Teknis Fase 4 (7 poin, termasuk isolasi RLS `tasks_scoped`/`subtasks_scoped` dari akun panitia yang belum pernah dites sejak dibuat di Fase 1). Kalau semua poin itu sukses, centang checkbox Fase 4 di atas lalu baru mulai sesi Fase 5 (Download/Submit Template Budgeting) — perhatikan juga catatan keputusan "bucket Budgeting diperlakukan sama seperti bucket biasa di Fase 4" di bawah, karena itu relevan buat desain Fase 5. Akun test `panitia-a@test.local`/`panitia-b@test.local` sekarang sudah tersambung ke 2 instance asli (bukan instance test yang sudah dihapus) — dipakai lagi untuk verifikasi RLS Fase 4 di atas tanpa perlu setup ulang, lihat `supabase/verify_fase3.sql` untuk instance mana yang jadi rujukannya.
