...
**Fase berikutnya yang harus dikerjakan: Fase 4 — Bucket Tugas & Breakdown.** Fase 3 sudah selesai & terverifikasi live sepenuhnya termasuk isolasi RLS dari akun panitia, tidak ada yang menggantung. Sesi Fase 4 sebaiknya mulai dari branch `main` setelah PR Fase 3 (`claude/project-setup-fase-3-l58rjm` → `main`) di-merge — cek dulu section 6 & Catatan Teknis Fase 3 di bawah untuk konteks bucket (termasuk keputusan hapus bucket "Penetapan Susunan Kepanitiaan" — jadi Fase 4 sebaiknya menyediakan tempat/cara bagi leader untuk input tugas "penetapan susunan kepanitiaan" itu sebagai task biasa, bukan bidang tersendiri, kalau memang masih relevan dicatat sebagai tugas). Akun test `panitia-a@test.local`/`panitia-b@test.local` sekarang sudah tersambung ke 2 instance asli (bukan instance test yang sudah dihapus) — bisa dipakai lagi buat verifikasi RLS di fase-fase berikutnya tanpa perlu setup ulang, lihat `supabase/verify_fase3.sql` untuk instance mana yang jadi rujukannya.
